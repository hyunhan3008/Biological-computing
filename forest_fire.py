...
def transition_func(grid, neighbourstates, neighbourcounts, decaygrid, randomgrid, terraingrid, probabilitygrid, windgrid, delaygrid):
    ...
    unburnt_cells = ((grid == 0) | (grid == 2)) # cells currently in state 3
    ...
    #'''
    wind_direction = (N > 3)
    windgrid[wind_direction] += 1.2
    
    wind_direction2 = (NW > 3)
    windgrid[wind_direction2] += 0.6
    
    wind_direction3 = (NE > 3)
    windgrid[wind_direction3] += 0.6
    
    wind_direction4 = (SW > 3)
    windgrid[wind_direction4] += 0.4
    
    wind_direction5 = (SE > 3)
    windgrid[wind_direction5] += 0.4
    
    opposite_direction6 = (S > 3)
    windgrid[opposite_direction6] -= 2.2

    opposite_direction6 = (W > 3)
    windgrid[opposite_direction6] -= 0.24

    opposite_direction6 = (E > 3)
    windgrid[opposite_direction6] -= 0.24
    #'''
    ...
    opposite_direction6 = (NW > 3)
    windgrid[opposite_direction6] -= 2.2

    opposite_direction7 = (SW > 3)
    windgrid[opposite_direction7] -= 0.24
 
    opposite_direction8 = (NE > 3)
    windgrid[opposite_direction8] -= 0.24
    '''
    ...
    delay_and_wind = np.multiply(random_and_wind, delaygrid)
    random_and_delay = np.multiply(combustion_neighbours, delaygrid)
...
"hello"
    ...
    '''
    ignitable_terrain = ((terraingrid != 1) & (terraingrid !=3))
    has_three_neighbours = (neighbourcounts[6]) >= 4
    has_three_burnt = (neighbourcounts[7]) >= 4
    ignited_chap = (((decaygrid == 0) & (random_and_wind > 0.35) & at_least_1_neighbour & ignitable_terrain & (delay_and_wind > 0.35))) | ((decaygrid == 0) & (random_and_delay >= 7) & ignitable_terrain) | has_three_neighbours & ignitable_terrain | has_three_burnt & ignitable_terrain
    ...
    forest_ignition = (decaygrid == 150) & (terraingrid == 2)
    ...
    chaparral_combustion = (decaygrid > 10) & (terraingrid == 0)
    grid[chaparral_combustion] = 6
    
    forest_combustion = (decaygrid > 250) & (terraingrid == 2)
    ...
    chaparral_extinction = (decaygrid > 82) & (terraingrid == 0)
    grid[chaparral_extinction] = 7
    
    forest_extinction = (decaygrid > 1440) & (terraingrid == 2)
...
def main():
   ...
   # gollum lives in this forest
    ...
    # Create grid object
    grid = Grid2D(config, (transition_func, decaygrid, randomgrid, terraingrid, probabilitygrid, windgrid, delaygrid))
